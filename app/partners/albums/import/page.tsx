...
export default function ImportAlbumPage() {
  ...
  async function createFromFiles(items: { photo: File; video: File; name: string }[]) {
    ...
    const res = await fetch('/api/partner/albums', {
      ...
      body: JSON.stringify({ items: keys, targetKey, customerName: albumName.trim() }),
    })
  ...
  async function handleDriveImport() {
    ...
    try {
      ...
      const cr = await fetch('/api/partner/albums', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ items: items.map((i) => ({ photoKey: i.photoKey, videoKey: i.videoKey })), targetKey, customerName: albumName.trim() }),
      })
  ...
  if (result) {
    return (
      <div className="mx-auto w-full max-w-2xl px-6 py-16">
        ...
        <p className="mt-2 text-cream/70">One QR for all {result.count} photos.</p>
      ...
      <section className="mt-6 rounded-3xl border border-cream/15 bg-green-mid/40 p-6">
        ...
        {mode === 'drive' ? (
          <div className="mt-5">
            ...
            <p className="mt-2 text-xs text-cream/50">The customer must share this folder with the service-account email (see setup notes).</p>
