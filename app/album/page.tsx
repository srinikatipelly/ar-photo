...
import { compileImageTargets } from '../upload/compile'

// Internal builder for album mode: many photo+video pairs behind ONE QR.
// Compiles all photos into a single MindAR .mind (browser-only), uploads assets,
// creates the album row, and shows the one QR. Not part of the paid order flow yet
// (see album-mode-plan.md).
...
export default function AlbumBuilderPage() {
  ...
  function addPair() {
    setPairs((prev) => (prev.length >= MAX_PAIRS ? prev : [...prev, emptyPair()]))
  }
  ...
  return (
    <main className="mx-auto flex min-h-screen w-full max-w-3xl flex-col px-4 py-16">
      <Link href="/" className="mb-8 inline-flex items-center gap-1.5 text-sm text-zinc-500 hover:text-zinc-900">← Back</Link>

      <span className="text-sm font-semibold uppercase tracking-[0.3em] text-amber-500">Album (test)</span>
      <h1 className="mt-3 text-3xl font-semibold tracking-tight text-zinc-900">Build an AR album</h1>
      <p className="mt-3 text-zinc-500">
        Add up to {MAX_PAIRS} photo + video pairs. They compile into <strong>one QR</strong> — scan once and pan
      ...
      <section className="mt-6 rounded-3xl border border-zinc-200 bg-white p-6 shadow-sm">
        ...
        {pairs.length < MAX_PAIRS && (
          <button onClick={addPair} className="mt-4 w-full rounded-xl border border-dashed border-zinc-300 px-4 py-3 text-sm font-medium text-zinc-600 hover:bg-zinc-50">
            + Add another photo ({pairs.length}/{MAX_PAIRS})
          </button>
        )}
